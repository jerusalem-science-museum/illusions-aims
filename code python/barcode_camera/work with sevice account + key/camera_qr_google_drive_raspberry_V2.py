...
# Sur Pi, 30 fps Tkinter + PIL = souvent trop. 15 fps = plus fluide.
PREVIEW_FPS = 15
...
class OverlayCache:
    ...
    def _load(self):
        ...
        if lg is not None:
            ...
            self.logo_x, self.logo_y = x, y

    def apply(self, frame_bgr):
        if self.frame_rgba is not None:
            frame_bgr = overlay_rgba(frame_bgr, self.frame_rgba, 0, 0)
        if self.logo_rgba is not None:
            frame_bgr = overlay_rgba(frame_bgr, self.logo_rgba, self.logo_x, self.logo_y)
        return frame_bgr
...
# =========================================================
# THREAD: grabber caméra (évite que Tkinter soit bloqué)
# =========================================================
...
class CameraAppGUI:
    ...
    def update_preview(self):
        ...
        frame = self.grabber.get_latest_copy()
        if frame is not None:
            small = cv2.resize(frame, (PREVIEW_W, PREVIEW_H), interpolation=cv2.INTER_LINEAR)
    ...
    def capture_flow(self):
        try:
            ...
            # overlays depuis cache (rapide)
            frame = self.overlays.apply(frame)
