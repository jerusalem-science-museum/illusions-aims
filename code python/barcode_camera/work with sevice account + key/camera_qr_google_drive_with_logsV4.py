...
LOGO_SCALE = 0.30  # fraction of the image width used for logo width
...
class CameraAppGUI:
    ...
    def __init__(self):
        ...
        # QR strip (bottom)
        self.qr_canvas = tk.Canvas(main, highlightthickness=0)
    ...
    def _apply_layout(self):
        ...
        # QR size adapts to window width (fits QR_HISTORY items + gaps)
        gap = int(self.qr_gap)
        max_qr = int((win_w - (QR_HISTORY + 1) * gap) / max(1, QR_HISTORY))
        # Keep QR readable but not huge
        qr_size = max(80, min(320, max_qr))
        ...
        self.qr_canvas.config(width=win_w, height=qr_canvas_h)
    ...
    def _qr_target_centers(self):
        ...
        # Use the actual canvas width so it stays centered
        total = QR_HISTORY * size + (QR_HISTORY + 1) * gap
        left = max(0, int((w - total) / 2))
    ...
    def update_preview(self):
        ...
        if ok and frame is not None:
            ...
            # Live preview stays CLEAN (no decorative overlays). Overlays are burned ONLY into the saved/uploaded photo.
